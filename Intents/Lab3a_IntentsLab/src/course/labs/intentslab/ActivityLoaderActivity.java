...
public class ActivityLoaderActivity extends Activity {
	...
	private void startExplicitActivation() {
		...
		try {
			Class ourClass = Class.forName("course.labs.intentslab.ExplicitlyLoadedActivity");
			explicitIntent = new Intent(ActivityLoaderActivity.this,ourClass);
	...
	private void startImplicitActivation() {
		...
		Intent chooserIntent = Intent.createChooser(baseIntent, TAG);
       // chooserIntent.createChooser(baseIntent, TAG);
        
		Log.i(TAG,"Chooser Intent Action:" + chooserIntent.getAction());
		PackageManager packageManager = getPackageManager();
		List activities = packageManager.queryIntentActivities(baseIntent,
		        PackageManager.MATCH_DEFAULT_ONLY);
		int intentSize = activities.size();
		Log.i("APP NUM", Integer.toString(intentSize));
        
		// TODO - Start the chooser Activity, using the chooser intent
        try{
		startActivity(chooserIntent);
        }catch(Exception e){
        	Log.i("IMPLICIT", e.toString());
        	e.printStackTrace();
        }
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if((resultCode == RESULT_OK)&&(requestCode== GET_TEXT_REQUEST_CODE)){
			returnString = data.getStringExtra("USERSTRING");
			Log.i("INSIDE RESULT", returnString);
			mUserTextView.setText(returnString);
		}		
		}catch(Exception e){
